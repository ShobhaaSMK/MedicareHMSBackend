...
async function runMigration() {
  try {
    console.log('Starting migration: Replace EmergencyBedSlotId with EmergencyAdmissionId in PatientICUAdmission table...\n');

    const migrationFilePath = path.join(__dirname, 'migrations', 'replace_emergencybedslotid_with_emergencyadmissionid_in_patienticuadmission.sql');
    ...
    `);
    console.log(`EmergencyAdmissionId foreign key constraint exists: ${checkFK.rows.length > 0 ? 'YES ✓' : 'NO (should be added)'}`);
