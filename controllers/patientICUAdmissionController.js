...
const mapPatientICUAdmissionRow = (row) => ({
  ...
  AttendingDoctorId: row.AttendingDoctorId !== undefined && row.AttendingDoctorId !== null ? parseInt(row.AttendingDoctorId, 10) : (row.attendingdoctorid !== undefined && row.attendingdoctorid !== null ? parseInt(row.attendingdoctorid, 10) : null),
  // Joined fields
...
const validatePatientICUAdmissionPayload = (body, requireAll = true) => {
  ...
  if (body.AttendingDoctorId !== undefined && body.AttendingDoctorId !== null) {
    const attendingDoctorIdInt = parseInt(body.AttendingDoctorId, 10);
    if (isNaN(attendingDoctorIdInt)) {
      errors.push('AttendingDoctorId must be a valid integer');
    }
  }

  if (body.PatientType !== undefined && body.PatientType !== null) {
    if (!allowedPatientTypes.includes(body.PatientType)) {
      errors.push('PatientType must be one of: OPD, IPD, Emergency, Direct');
    }
  }
...
exports.createPatientICUAdmission = async (req, res) => {
  try {
    ...
    if (EmergencyAdmissionId !== undefined && EmergencyAdmissionId !== null) {
    ...
    let attendingDoctorIdValue = null;
    if (AttendingDoctorId !== undefined && AttendingDoctorId !== null && AttendingDoctorId !== '') {
      const attendingDoctorIdInt = parseInt(AttendingDoctorId, 10);
      if (isNaN(attendingDoctorIdInt)) {
        return res.status(400).json({ success: false, message: 'AttendingDoctorId must be a valid integer.' });
      }
      const doctorExists = await db.query('SELECT "UserId" FROM "Users" WHERE "UserId" = $1', [attendingDoctorIdInt]);
      if (doctorExists.rows.length === 0) {
        return res.status(400).json({ success: false, message: 'AttendingDoctorId does not exist.' });
      }
      attendingDoctorIdValue = attendingDoctorIdInt;
    }
...
exports.updatePatientICUAdmission = async (req, res) => {
  try {
    ...
    if (AttendingDoctorId !== undefined && AttendingDoctorId !== null && AttendingDoctorId !== '') {
      ...
      attendingDoctorIdValue = attendingDoctorIdInt;
    } else if (AttendingDoctorId === null) {
      attendingDoctorIdValue = null;
    }
    ...
    if (RoomAdmissionId !== undefined) {
      ...
      params.push(RoomAdmissionId !== null ? parseInt(RoomAdmissionId, 10) : null);
    }
    if (PatientType !== undefined) {
      updates.push(`"PatientType" = $${paramIndex++}`);
      params.push(PatientType !== null && PatientType !== '' ? PatientType.trim() : null);
    }
    if (ICUId !== undefined) {
    ...
    if (AttendingDoctorId !== undefined) {
      updates.push(`"AttendingDoctorId" = $${paramIndex++}`);
      params.push(attendingDoctorIdValue);
    }
...
// ============================================
// NEW IMPLEMENTATION - REGENERATED
// ============================================
/**
 * GET /api/patient-icu-admissions/icu-beds-details/:id
 * Get ICU bed details with all admission records for a specific ICUId
 * @param {Object} req - Express request object
 * @param {Object} req.params - Request parameters
 * @param {string} req.params.id - ICUId (must be a number)
 * @param {Object} res - Express response object
 */
exports.getICUBedsDetailsMgmtByICUId = async (req, res) => {
  try {
    console.log('[getICUBedsDetailsMgmtByICUId] Called with id:', req.params.id);
    
    // Extract ICUId from path parameters - lenient validation
    const { id } = req.params;
    const icuId = parseInt(id, 10);
    const queryId = isNaN(icuId) ? id : icuId;
    
    console.log('[getICUBedsDetailsMgmtByICUId] Query ID:', queryId);

    // Simplified query - get ICU first, then admissions separately
    const icuQuery = `
      SELECT 
        "ICUId",
        "ICUBedNo",
        ...
        "ICUEndTimeofDay",
        "Status" AS "ICUStatus",
        "CreatedBy" AS "ICUCreatedBy",
        "CreatedAt" AS "ICUCreatedAt"
      FROM "ICU"
      WHERE "ICUId" = $1
      LIMIT 1
    `;

    console.log('[getICUBedsDetailsMgmtByICUId] Fetching ICU bed...');
    const icuResult = await db.query(icuQuery, [queryId]);
    console.log('[getICUBedsDetailsMgmtByICUId] ICU query returned', icuResult.rows.length, 'rows');

    if (icuResult.rows.length === 0) {
      return res.status(200).json({
        success: true,
        message: 'ICU bed details retrieved successfully',
        data: {
          icuId: queryId,
          icuBedNo: null,
          icuType: null,
    ...
    console.log('[getICUBedsDetailsMgmtByICUId] Fetching admissions...');
    const admissionsResult = await db.query(admissionsQuery, [queryId]);
    console.log('[getICUBedsDetailsMgmtByICUId] Admissions query returned', admissionsResult.rows.length, 'rows');
    
    const rows = admissionsResult.rows;

    // Format ICU bed details
    const icuBedDetails = {
      icuId: icuBed.ICUId,
      ...
      icuStartTimeofDay: icuBed.ICUStartTimeofDay,
      icuEndTimeofDay: icuBed.ICUEndTimeofDay,
      icuStatus: icuBed.ICUStatus,
      icuCreatedBy: icuBed.ICUCreatedBy,
      icuCreatedAt: icuBed.ICUCreatedAt
    };

    // Format admission records
    const admissions = rows.map(row => ({
          patientICUAdmissionId: row.PatientICUAdmissionId || null,
          ...
          icuAllocationFromDate: row.ICUAllocationFromDate || null,
          icuAllocationToDate: row.ICUAllocationToDate || null,
          numberOfDays: row.NumberOfDays || null,
          diagnosis: row.Diagnosis || null,
          treatementDetails: row.TreatementDetails || null,
          patientCondition: row.PatientCondition || null,
          icuAllocationCreatedBy: row.ICUAllocationCreatedBy || null,
          icuAllocationCreatedAt: row.ICUAllocationCreatedAt || null,
          admissionStatus: row.AdmissionStatus || null,
          onVentilator: row.OnVentilator || null,
          attendingDoctorId: row.AttendingDoctorId !== undefined && row.AttendingDoctorId !== null ? parseInt(row.AttendingDoctorId, 10) : (row.attendingdoctorid !== undefined && row.attendingdoctorid !== null ? parseInt(row.attendingdoctorid, 10) : null),
          // Patient Details
          patientName: row.PatientName || null,
          patientNo: row.PatientNo || null,
          patientAge: row.Age || null,
          patientGender: row.Gender || null,
          patientPhoneNo: row.PatientPhoneNo || null,
          // Appointment Details
          appointmentTokenNo: row.AppointmentTokenNo || null,
    ...
    console.log('[getICUBedsDetailsMgmtByICUId] Sending response with', admissions.length, 'admissions');
    return res.status(200).json({
      success: true,
      message: 'ICU bed details with admission records retrieved successfully',
      data: responseData
    ...
    console.error('Error in getICUBedsDetailsMgmtByICUId:', error);
  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        -- Attending Doctor Details
        attendingDoctor."UserName" AS "AttendingDoctorName",
        attendingDoctor."EmailId" AS "AttendingDoctorEmail",
        attendingDoctor."PhoneNo" AS "AttendingDoctorPhone"
      FROM "PatientICUAdmission" pica
      INNER JOIN "ICU" icu ON pica."ICUId" = icu."ICUId"
    ...
    const formattedData = {
      ...
      // Attending Doctor Details
      attendingDoctor: row.AttendingDoctorId ? {
        userId: row.AttendingDoctorId !== undefined && row.AttendingDoctorId !== null ? parseInt(row.AttendingDoctorId, 10) : (row.attendingdoctorid !== undefined && row.attendingdoctorid !== null ? parseInt(row.attendingdoctorid, 10) : null),
        userName: row.AttendingDoctorName || row.attendingdoctorname || null,
        emailId: row.AttendingDoctorEmail || row.attendingdoctoremail || null,
        phoneNo: row.AttendingDoctorPhone || row.attendingdoctorphone || null
      } : null
    };
